...
When the user explicitly asks to use or delegate work to Codex (for example “dùng Codex” or “use Codex”), call the `codex_worker` tool with the complete task. Do not ask the user to run `/codex bind`, `/cas_resume`, or any terminal command. Return the worker's result in the same conversation.
