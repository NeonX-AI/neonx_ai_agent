...
import { createLeaderbotAiAnswerIdempotencyKey, finalizeLeaderbotAiAnswerQuota, isLeaderbotAiAnswerEnforcementEnabled, reserveLeaderbotAiAnswerQuota, } from "./leaderbot-answer-quota.js";
...
const STARTPILOT_AI_ANSWER_QUOTA_REPLY = "Je Startpilot-tegoed van 300 AI-antwoorden is opgebruikt. Bekijk je tegoed via https://leaderbot.live/?upgrade=startpilot#pricing";
const STARTPILOT_AI_ANSWER_QUOTA_UNAVAILABLE_REPLY = "Leaderbot kan je tegoed nu niet veilig controleren. Probeer zo meteen opnieuw.";
...
function messengerPromptSenderKey(scope) {
    return JSON.stringify([scope.accountId, scope.pageId, scope.senderId]);
}
function messengerPromptMessageKey(scope, messageId) {
...
export function rememberMessengerAssistantPrompt(params) {
    ...
    if (normalizedMessageId) {
        recentMessengerAssistantRepliesByMessage.set(messengerPromptMessageKey(params, normalizedMessageId), { text: params.text, expiresAt });
    }
    if (!prompt) {
        return;
    }
    ...
    if (normalizedMessageId) {
        recentMessengerAssistantPromptsByMessage.set(messengerPromptMessageKey(params, normalizedMessageId), { prompt, expiresAt });
    }
}
function resolveRememberedMessengerAssistantPrompt(params) {
    const now = params.now ?? Date.now();
    pruneRecentMessengerAssistantPrompts(now);
    const normalizedMessageId = params.replyToMessageId?.trim();
    if (normalizedMessageId) {
        const exact = recentMessengerAssistantPromptsByMessage.get(messengerPromptMessageKey(params, normalizedMessageId))?.prompt;
        if (exact) {
            return exact;
        }
    }
...
function resolveMessengerAssistantReplyOptionPrompt(params) {
    ...
    const normalizedMessageId = params.replyToMessageId?.trim();
    if (normalizedMessageId) {
        const exactAssistantReply = recentMessengerAssistantRepliesByMessage.get(messengerPromptMessageKey(params, normalizedMessageId))?.text;
        if (exactAssistantReply) {
            return extractNumberedImageOptionFromAssistantReply(exactAssistantReply, params.text);
        }
    }
    const assistantReply = recentMessengerAssistantReplies.get(messengerPromptSenderKey(params))?.text;
...
export async function processMessengerEvent(params) {
    ...
    try {
        ...
        const core = getMessengerRuntime();
        let aiAnswerQuotaReservationId = null;
        if (isLeaderbotAiAnswerEnforcementEnabled()) {
            ...
            if (quotaDecision.status === "unavailable") {
                await sendMessengerText(senderId, STARTPILOT_AI_ANSWER_QUOTA_UNAVAILABLE_REPLY, {
                    cfg: params.cfg,
                    accountId: params.account.accountId,
                });
                return;
        ...
        if (aiAnswerQuotaReservationId) {
            ...
            if (!finalized) {
                params.runtime.error?.(danger(`messenger paid AI answer quota finalization failed outcome=${outcome} account=${params.account.accountId}`));
            }
        }
        if (openClawError) {
            throw openClawError;
